...
export default (sender, msg) => {
  ...
  if (!Number.isNaN(amountOfKeys) && amountOfKeys > 0) {
    log.userChat(sender.getSteamID64(), `[ !BUYHYDRA ${amountOfKeys} ]`);
    chatMessage(sender, messages.request);

    const myCards = [];

    for (let i = 0; i < stock.botEventCards.length; i += 1) {
      if (myCards.length < amountOfKeys * rates.buy.hydra) {
          ...
          if (!error) {
            ...
            if (theirKeys.length !== amountOfKeys) {
              ...
              const message = messages.trade.message.hydra[1]
                ...
                .replace('{EVENTCARDS}', amountOfKeys * rates.buy.hydra);
    ...
    chatMessage(
      sender,
      messages.error.inputinvalid.keys.replace('{command}', '!BUYHYDRA 1')
    );
  }
};
