...
export default (sender, msg) => {
  ...
  if (!Number.isNaN(amountOfKeys) && amountOfKeys > 0) {
    log.userChat(sender.getSteamID64(), `[ !BUYTF ${amountOfKeys} ]`);
    chatMessage(sender, messages.request);

    const myCards = [];

    for (let i = 0; i < stock.botEventCards.length; i += 1) {
      if (myCards.length < amountOfKeys * rates.buy.tf) {
          ...
          if (!error) {
            ...
            if (theirKeys.length !== amountOfKeys) {
              ...
              const message = messages.trade.message.tf[1]
                ...
                .replace('{EVENTCARDS}', amountOfKeys * rates.buy.tf);
    ...
    chatMessage(
      sender,
      messages.error.inputinvalid.keys.replace('{command}', '!BUYTF 1')
    );
  }
};
