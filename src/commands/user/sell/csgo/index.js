...
export default (sender, msg) => {
  ...
  if (!Number.isNaN(amountOfKeys) && amountOfKeys > 0) {
    log.userChat(sender.getSteamID64(), `[ !SELLCSGO ${amountOfKeys} ]`);
    ...
    getEventCards(sender.getSteamID64(), async (error1, inv) => {
      if (!error1) {
        for (let i = 0; i < inv.length; i += 1) {
          if (theirCards.length < amountOfKeys * rates.sell.csgo) {
              ...
              if (!error2) {
                ...
                if (myKeys.length !== amountOfKeys) {
                  ...
                  const message = messages.trade.message.csgo[1]
                    ...
                    .replace('{EVENTCARDS}', amountOfKeys * rates.sell.csgo);
    ...
    chatMessage(
      sender,
      messages.error.inputinvalid.keys.replace('{command}', '!SELLCSGO 1')
    );
  }
};
