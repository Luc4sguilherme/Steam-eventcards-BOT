...
export default (sender, msg) => {
  ...
  if (!Number.isNaN(amountOfKeys) && amountOfKeys > 0) {
    log.userChat(sender.getSteamID64(), `[ !SELLTF ${amountOfKeys} ]`);
    ...
    getEventCards(sender.getSteamID64(), async (error1, inv) => {
      if (!error1) {
        for (let i = 0; i < inv.length; i += 1) {
          if (theirCards.length < amountOfKeys * rates.sell.tf) {
            ...
            (error2, keys) => {
              if (!error2) {
                for (let i = 0; i < keys.length; i += 1) {
                  if (currencies.tf.includes(keys[i].market_hash_name)) {
                ...
                if (myKeys.length !== amountOfKeys) {
                  ...
                  const message = messages.trade.message.tf[1]
                    .replace('{KEYS}', amountOfKeys)
                    .replace(/{EVENTNAME}/g, main.eventName)
                    .replace('{EVENTCARDS}', amountOfKeys * rates.sell.tf);
    ...
    chatMessage(
      sender,
      messages.error.inputinvalid.keys.replace('{command}', '!SELLTF 1')
    );
  }
};
