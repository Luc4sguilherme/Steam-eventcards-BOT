...
export default (sender, msg) => {
  ...
  const command =
    input.match('!DEPOSITHYDRA') ||
    input.match('!HYDRADEPOSIT') ||
    input.match('!HYDP') ||
    [];
  const amountkeys = parseInt(input.replace(`${command[0]} `, ''), 10);

  if (!Number.isNaN(amountkeys) && amountkeys > 0) {
    log.adminChat(sender.getSteamID64(), `[ !DEPOSITHYDRA ${amountkeys} ]`);
        ...
        if (!error) {
          ...
          for (let i = 0; i < inv.length; i += 1) {
            if (
              ...
              currencies.hydra.indexOf(inv[i].market_hash_name) >= 0
            ) {
